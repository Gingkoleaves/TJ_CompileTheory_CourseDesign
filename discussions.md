...
`analyze_program` 不检查程序中是否存在 `main` 函数；上述源代码不会报任何错误，
下游 IR 也就没有 `main` 入口、相当于一个"库"而非"二进制"。

### Rust 严格语义

`rustc` 对 binary crate 强制要求 `fn main()`，缺失即报
`error[E0601]: main function not found`。`main` 还要求参数 0 个、返回 `()` 或
实现 `Termination` 的类型（本课程范围内通常即 `i32`）。

### 当前取舍的依据

- PDF 规则 1.1–9.3 没有显式列出"必须存在 main"这一约束；
- 评分主要看 IR 形态而非可执行性；保留宽松也不会让"非法程序通过"——只是允许
  "无入口程序通过"，这在测试拼装阶段反而方便（可以只测一个函数）。

### 改严格的成本

- 几行代码：`analyze_program` 末尾扫描 `functions` 表，若不存在名为 `main`
  的项即报错；进一步可校验参数列表为空、返回类型为 `Unit` 或 `i32`。
- 影响面：现有单函数最小测试样例（不少 cand 探针只有一个 `fn`）会全部失败，
  需要批量补 `fn main(){}` 壳。
...
- 倾向"保留宽松"：测试样例尺寸小、不引入失败级联是更划算的工程取舍。
- 若答辩被问及，可指本条 D-8：与 PDF 规则未强制相吻合；如评分确需强制，
  在 `analyze_program` 收尾处加一段扫描即可，几乎零代价。

---

## D-9 `Unit` 条件 / `Error` 条件仍发 `IF_FALSE _ _ L2`
...
会输出错误，但 IR 中仍照常发射 `IF_FALSE _ _ L2`——第二元是 `_`（占位符）
当作 cond 槽位。

### 当前取舍的依据

- 与 D-4 / D-7 一致：错误恢复阶段保留 IR 形态，让下游解释器看到完整骨架；
- 占位符 `_` 在解释器侧通常被解读为 0，落入 `IF_FALSE` 即"总跳到 end"，
  不会触发 panic；
- 拆掉这条 `IF_FALSE` 与对应 `LABEL` 也需要同步处理 then/else 块的 IR 拼装
  与 `LABEL` 表，复杂度高于收益。

### 改严格的成本

- 在发射 `IF_FALSE` 之前检查 `cond.ty.is_known() && (cond.ty == Bool || cond.ty == I32)`；
- 否则跳过 `IF_FALSE` 与配套 `LABEL`、仍执行 then 块（按"错误恢复继续分析"），
  并把 if 表达式整体置为 `Type::Error`；
- 影响面：所有已有"条件类型错误"相关测试的 IR 断言都要重新刷新。

### 决策点

**是否在条件类型不合法时不发 `IF_FALSE`？**

- 倾向"保留宽松"：D-4 / D-7 都已定调"诊断与代码生成解耦"，此条与之同源。
- 若答辩被问及"为什么报错了还要发 `IF_FALSE`？"，可指本条 D-9：错误恢复
  优先保 IR 形态稳定，下游解释器会把占位符当 0 安全跳过。

---

## D-10 Lexer 按字节而非字符计列号，多字节 UTF-8 后位置偏移

### 现状

- 位置：`Easy_Lexer/src/lib.rs:361-371`（`advance`）
- 源码 `中a`，错误位置 `(1,1)`、`(1,2)`、`(1,3)`（"中"3 个字节各报一次 UTF-8 未识别错误），随后 `a` 出现在 `(1,4)`；期望 `(1,2)`（按字符计）。

### Rust 严格语义

- rustc 列号按 `char` 计。诊断信息里 `column` 与人眼看到的字符位置一致。

### 当前取舍的依据

- 实际编译目标是 ASCII 源码，多字节字符仅可能出现在注释 / 字符串字面量中，
  正常程序的语法位置不会落在 UTF-8 序列内部；
- 现有大量 lexer 错误测试断言基于"按字节列号"，改为按字符计需要回归刷新；
- 错误信息中的 `column` 偏移在 ASCII 路径下完全等价，不影响真实程序定位。

### 改严格的成本

- 用 `char_indices` 或字符宽度计算替换 `advance` 内的 `self.column += 1`；
- 现有 lexer 错误测试列号要逐条校准；
- 与 parser / analyzer 用到的 `Position.column` 字段语义同步。

### 决策点

**是否把列号改成按字符计？**

- 倾向"保留按字节"：中文出现在注释 / 字符串字面量外的场景极少，
  实际程序基本是 ASCII；改严格的回归代价大于诊断收益。
- 若答辩被问及"中文程序错位"，可指本条 D-10：与 D-1 / D-2 / D-8 同源——
  教学语义偏窄，主线 ASCII 程序无影响。

---

## D-11 Parser 链式比较 `a<b<c` 被接受

### 现状

- 位置：`Easy_Parser/src/lib.rs:549-562`（`parse_expression` 内用 `while let` 累积比较运算）
- `a<b<c` 解析为 `(a<b)<c`，到了语义阶段才报"`<` 仅支持 i32"
  （因为 `(a<b)` 类型是 `bool`，再和 `c` 做 `<` 就会被语义检查拦截）。

### Rust 严格语义

- rustc 在 parse 层就拒绝链式比较（`comparison operators cannot be chained`），
  并提示用 `&&` 显式连接。

### 当前取舍的依据

- 语义阶段已经能稳定拦截 `bool < i32`；
- 改 `while` 为 `if` 会破坏现有比较运算 parse 测试的预期（部分测试断言"`a<b<c` 能 parse 出三元 AST"）。

### 改严格的成本

- 极低（一处 `while` → `if`），但需要同步刷新 parser / analyzer 双侧测试。

### 决策点

**是否在 parse 层直接拒绝链式比较？**

- 倾向"保留语义层兜底"：与 D-3 / D-9 同源——能在语义阶段以更精准的类型信息报错，
  不必在 parse 阶段加特例；
- 若答辩被问及，可指本条 D-11：parse 层保持简单的左结合栈，
  语义阶段统一做"运算数类型限定"。
...
- 源码 `fn f()->&i32 { let x:i32=1; return &x; }` 当前 0 错；
  Rust 报 `cannot return reference to local variable`。

### Rust 严格语义

- rustc 通过借用检查器 + 生命周期推断捕获返回局部引用 / 悬垂引用。

### 当前取舍的依据

- 本课程实现无生命周期 / 借用作用域追踪，PDF 规则集也未要求；
- 与 D-1（无借用检查器）/ D-2（无 Drop 时机分析）一脉相承：教学语义偏松，
  不引入借用域追踪。

### 改严格的成本

- 极高：需要给每个 `&x` 表达式打"来源作用域深度"标签，
  在 `gen_return` 时核对返回引用所指深度是否浅于函数入口；
- 进而衍生出生命周期约束，对整个表达式 / 语句的类型系统都有传染性。

### 决策点

**是否引入"返回引用作用域检查"？**

- 倾向"保留宽松"：超出 PDF 规则覆盖面，与 D-1 / D-2 同源。
- 若答辩被问及，可指本条 D-12：教学语义不做借用 / 生命周期追踪，
  返回局部引用的运行期错误由"教学约束：不要这样写"兜底。

---

## D-13 尾随逗号在参数列表 / 元组类型中被拒

### 现状

- 位置：`Easy_Parser/src/lib.rs:313-333`（`parse_parameter_list`）
- 源码 `fn f(a:i32,) {}` parse error，提示"expected parameter"或类似。

### Rust 严格语义

- Rust 允许尾随逗号，包括参数列表、元组、数组字面量等所有列表语法。

### 当前取舍的依据

- 课程 PDF 文法 1.1 写的是 `params -> param (, param)*`，没有把 `,)` 单列为合法尾巴；
- 教学样例不会习惯性写尾随逗号，与"严格按文法实现"保持一致。

### 改严格的成本

- 单点改动极低（在 `parse_parameter_list` 的循环里容忍尾部多一个 `,`）；
- 但若要保持一致体验，需要同步放宽 tuple 字面量 / 数组字面量 / 元组类型注释的尾随逗号——
  涉及若干处 `parse_*` 函数。
...
- 倾向"保留偏严"：按 PDF 文法落地，与"严格按文法实现"保持口径一致；
- 若答辩被问及，可指本条 D-13：课程文法层未定义尾随逗号，
  与"未定义形式视为非法"保持口径。
...
如果未来引入"真·控制流分析"或"IR 输出可执行性保证"两条新需求，D-3 / D-4 / D-9
应作为对应里程碑里的真 BUG 一并修复；D-6 则在引入"常量折叠 / const-eval"
里程碑时与之同步落地；D-8 关乎评分形态，单独一行扫描即可补齐。
